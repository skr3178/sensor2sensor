...
What this does:
    ...
    2. Decodes one TOP-laser frame into a [3, 64, 2048] range image in [0, 1].
...
def main():
    ...
    print(f"  output shape : {x.shape}  (expected (3, {H_DEFAULT}, {W_DEFAULT}))")
    summarize_array("range channel    [0]", x[0])
    summarize_array("intensity channel[1]", x[1])
    summarize_array("validity channel [2]", x[2])
    fill = float(x[2].mean()) * 100
