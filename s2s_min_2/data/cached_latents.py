...
Returns dicts with `img_latents` (6 cams), `raymaps` (6 cams), `mu`, and
`sample_token`. M3's training loop and M4's inference loop both consume this
dataset.
...
class CachedLatentsDataset(Dataset):
    ...
    Each `__getitem__` returns a dict with float32 tensors:
        img_latents  : [V, 4, 32, 56]    (V cameras through the SD VAE)
        raymaps      : [V, 6, 32, 56]    (per-camera ego-frame raymap)
        mu           : [8, 8, 256]       (LiDAR VAE posterior mean; the diffusion target)
        sample_token : str               (for logging / debugging — not for the model)

    Canonical camera order is `data.nuscenes_mini_paired.CAMERA_ORDER`; if the
    cache was written with a different order the `camera_order` array in the
    .npz still carries the truth.
    ...
    def __init__(
        self,
        cache_dir: str | Path = "out/cached_latents",
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        npz = np.load(path)
        item = {
            "img_latents":  torch.from_numpy(npz["img_latents"]),
            "raymaps":      torch.from_numpy(npz["raymaps"]),
