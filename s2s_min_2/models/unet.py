...
from .attention import CrossAttention, CrossSensorSelfAttn, SelfAttention
...
class LiDARUNet(nn.Module):
    ...
    Args:
        ...
        use_cross_sensor:   if True, swap every CrossAttention block (one-way LiDAR→cam)
                            for CrossSensorSelfAttn (symmetric self-attn over
                            [lidar_tokens; camera_tokens]) per paper §3.2.3.
                            Default False (= Scope B). Enabling this changes the
                            checkpoint shape — Scope B and C checkpoints are not
                            interchangeable.
    """
    ...
    def __init__(
        ...
        self.enc_l0 = EncoderLevel(
            ...
            return_skip=True,
            use_cross_sensor=use_cross_sensor,
        )
